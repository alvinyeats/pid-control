...
*		串级控制系统的计算顺序是先主回路（PID1），后副回路（PID2）。控制方式有
*	两种：一种是异步采样控制，即主回路的采样控制周期T1是副回路采样控制周期T2的
*	整数倍。这是因为一般串级控制系统中主控对象的响应速度慢、副控对象的响应速度
*	快的缘故。另一种是同步采样控制，即主、副回路的采样周期相同。这时，应根据副
*	回路选择采样周期，因为副回路的受控对象的响应速度较快。
*
